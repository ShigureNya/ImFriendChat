...
public class ImageDownloader {
    ...
    public Bitmap downloadImage(final String url, final onImageLoaderListener listener){
        ...
        //我们没有创建此目录保存文件就会报错
        final String subUrl = url.substring(url.lastIndexOf("/"),url.length());
        final String saveUrl = ContentSave.saveMainDiskPath+subUrl;
        ...
        if(bitmap != null){
            ...
            getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    //保存在SD卡或者手机目录
                    BitmapUtils.saveBitmapToDisk(bitmap,saveUrl);

                    //将Bitmap 加入内存缓存
                    addBitmapToMemoryCache(saveUrl, bitmap);
                }
            });
        }
